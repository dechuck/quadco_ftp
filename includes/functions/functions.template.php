...
class template_compile {
	...
	*/
	function _tpl_load_file($handle, $store_in_db = false) {
		...
		if( !file_exists($this->template->files[$handle]) )
		  throw new exception("template->_tpl_load_file(): File {$this->template->files[$handle]} does not exist or is empty");

		  $this->template->compiled_code[$handle] = $this->compile(@file_get_contents($this->template->files[$handle]));
	...
	* @access private
	*/
	function compile($code, $no_echo = false, $echo_var = '') {	if( $echo_var )
		 global $$echo_var;
	...
	* @access private
	*/
	function compile_write($handle, $data) {	$filename = $this->template->cachepath . str_replace('/', '.', $this->template->filename[$handle]) . '.php';
